[**HOME**](README.md) | [**TOPUP**](https://meawtopup.github.io/) | [**DOWNLOAD**](DOWNLOAD.md) | [**MANUAL**](MANUAL.md) | [**FEATURE**](FEATURES.md)

# IPv6 TDD 1.0
ฺBatch File สามารถเปิดใช้ได้บน
- Windows 7  
- Windows 8/8.1  
- Windows 10 (ทดสอบโดยผู้เขียน)  
- Windows 11
รองรับเฉพาะ
- Chrome
- Edge
- Firefox

# Get Batch File
มี 2 วิธี ในการดาวโหลด IPv6_TDD_v1_0.bat
1. โหลดจาก GitHub: [Download ](tools/IPv6_TDD_v1_0.bat?download=true)
   (ต้อง Login GitHub ก่อน หรืออยากเช็คโค้ด [IPv6 TDD V1.0](tools/IPv6_TDD_v1_0.bat) )  
2. โหลดจาก MediaFire: [Download on MediaFire](https://www.mediafire.com/file/aqtbt3h03m09ao6/IPv6_TDD_v1_0.bat/file)

# Run Batch File
1. ให้ไปที่โฟเดอร์ดาวโหลดของบราวเซอร์ที่ตั้งค่าไว้
2. คลิกขวาที่ไฟล์ IPv6_TDD_v1_0.bat เลือก Run as administrator
3. บางเครื่องอาจจะมี Security Warning ให้เลือก Run
4. หน้าต่าง Cmd (Command Prompt) จะเปิดขึ้นมา ถ้าทำงานได้จะมีข้อความแบบนี้
   ```
   ==================================================
     IPv6 TDD v1.0
   ==================================================
     [1] Block IPv6 (Add Rules)
     [2] Unblock IPv6 (Delete Rules)
   ==================================================
     Created by: MObyEX
   ==================================================
   Select an option (1-2):
   ```  
   **หากปรากฎข้อความแบบนี้**  
   ```
   [Error] Please right-click and "Run as Administrator"
   Press any key to continue . . .
   ```  
   ให้ปิด CMD แล้วกลับไปทำข้อ 3 ใหม่  
   ถ้าทำแล้วยังขึ้นเหมือนเดิมอีก แสดงว่าวินโดที่ท่านใช้ไม่มีสิทธิ์แก้ไขระดับแอดมิน
6. ถ้าต้องการ Block IPv6 ให้กด 1 ถ้าจะยกเลิกการ Block IPv6 ให้กด 2
7. ปิด Cmd ได้เลย
8. ทดลองปิดบราวเซอร์ ปิด VPN แล้วเปิดบราวเซอร์ใหม่ เข้าเว็บดู
   (ต้องแน่ใจว่าปิดทุกบราวเซอร์แล้ว หรือจะรีสตาร์ทเครื่องก็ได้)
