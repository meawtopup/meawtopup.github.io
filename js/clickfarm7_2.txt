// ==UserScript==
// @name         Click Farm 7.2 | TDDFarm
// @namespace    http://tampermonkey.net/
// @version      7.2
// @description  เพิ่มระบบออโต้ ใช้กับ New Windows เท่านั้น
// @author       MobyEX
...
(function() {
    ...
    if (cardBody) {
        ...
        if (isAuto) {
            ...
            let actionWhenDone = 'reload';

            if (hIdsInitial.length > 0 || pIdsInitial.length > 0) {
                remainingSeconds = 30;
                actionWhenDone = 'run';
            ...
            const targetTimeMs = Date.now() + (remainingSeconds * 1000);

            const timerInterval = setInterval(() => {
                const currentRemaining = Math.max(0, Math.round((targetTimeMs - Date.now()) / 1000));

                if (currentRemaining <= 0) {
                    clearInterval(timerInterval);
                    if (actionWhenDone === 'run') {
                        autoBtn.innerHTML = '🤖 กำลังสั่งเริ่มฤดูกาลใหม่...';
                        runNewSeason();
                    } else {
                        autoBtn.innerHTML = '🤖 กำลังรีเฟรชหน้าเว็บเพื่ออัปเดตสถานะ...';
                        window.location.reload();
                    }
                } else {
